...
# =============================================================================
# FIX RIASSUNTONE — ESTRAZIONE ROBUSTA DELLE SEZIONI
#   PROBLEMA RISOLTO: il vecchio codice prendeva SOLO il primo <p> dopo ogni
#   <h3> (find_next_sibling('p')). Se la sezione era "ricca" — più paragrafi,
#   liste <ul>/<ol>, blocchi <strong> che spezzano il testo — tutto ciò che
#   veniva dopo il primo paragrafo SPARIVA dal JSON (testo tagliato a metà,
#   sezioni che finivano con ":" o "(es."). In più get_text(strip=True)
#   incollava le parole attorno ai tag inline: "una<strong>valutazione</strong>"
#   diventava "unavalutazione".
#   SOLUZIONE: si cammina su TUTTI i fratelli tra un <h3> e il successivo,
#   raccogliendo paragrafi, div e liste (i cornell-box restano esclusi:
#   hanno la loro pipeline dedicata). Grassetti/corsivi vengono appiattiti
#   in testo standard con spazi corretti: il template legge tutto.
# =============================================================================
def testo_piatto(el):
    """get_text con separatore spazio: i tag inline (<strong>, <em>, <b>...)
    non incollano più le parole. Poi normalizza spazi e punteggiatura."""
    t = el.get_text(' ', strip=True)
    t = re.sub(r'\s+', ' ', t)
    t = re.sub(r'\s+([.,;:!?)\]])', r'\1', t)   # niente spazio prima di . , ) ]
    t = re.sub(r'([(\[])\s+', r'\1', t)          # niente spazio dopo ( [
    return t.strip()

def estrai_testo_sezione(h3):
    """Raccoglie TUTTO il testo tra questo <h3> e il prossimo <h3>:
    paragrafi, div generici, blockquote e liste. Ogni blocco viene chiuso
    con un punto se manca, così il chunking del template (che spezza sulle
    frasi) funziona anche sugli elenchi puntati appiattiti."""
    blocchi = []
    for sib in h3.find_next_siblings():
        if sib.name == 'h3':
            break  # inizia la sezione successiva
        classi = sib.get('class') or []
        if 'cornell-box' in classi:
            continue  # i check point sono gestiti dalla loro pipeline
        if sib.name in ('p', 'div', 'blockquote', 'h4', 'h5'):
            t = testo_piatto(sib)
            if t:
                blocchi.append(t if t[-1] in '.!?:' else t + '.')
...
if not html_files:
    ...
    def estrai_dati_master(html_content):
        ...
        # --- RIASSUNTONE ---
        # FIX — prima: testo = SOLO il primo <p> dopo l'h3 (find_next_sibling).
        # Sezioni con più paragrafi, liste o blocchi in grassetto venivano
        # TAGLIATE. Ora estrai_testo_sezione() raccoglie tutti i blocchi fino
        # al prossimo h3 e appiattisce la formattazione in testo standard.
        riassunto = soup.find(id='source-riassunto')
        if riassunto:
            for h3 in riassunto.find_all('h3'):
                testo = estrai_testo_sezione(h3)
                ...
                if cornell:
                    ...
                    for p in cornell.find_all('p'):
                        ...
                        testo_p_corrente = testo_piatto(p)
    ...
    for filename, contenuto in html_files.items():
        ...
        try:
            ...
            # --- DIAGNOSTICA RIASSUNTONE — mai più tagli silenziosi ---
            # Se una sezione esce sospettosamente corta (o finisce con ":"),
            # lo script lo segnala subito invece di lasciartelo scoprire online.
            for macro in dati["riassuntone"]:
                t = macro.get("testo", "")
                if len(t) < 120 or t.rstrip().endswith(':'):
                    print(f"   ⚠️ Sezione '{macro['titolo'][:50]}…' molto corta ({len(t)} caratteri): controlla la sorgente.")
